 ...
 *
 *         This program parses and interprets a simple programming output,
 *         assignments and switch statements. No loop.
 *
 *         <input> ::= <section> <section> ::= '[' <statement>* ']' <statement>
 *         ::= <assignment> | <switch> | <output> <assignment> ::= ID '=' (INT |
 *         ID) <output> ::= 'output' (INT | ID | STRING) <switch> ::= 'switch'
 *         ID '{' <case>* [ 'default' ':' <statement>* ] '}' <case> ::= 'case'
 *         INT ':' <statement>* 'break' ID ::= [a-zA-Z_]+ INT ::= [0-9]+ STRING
 *         starts and ends with double quotes ("), " can by escaped: (\")
 */
public class Fall20JavaProg {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // print the footer.
        String inputSource;
        inputSource = "http://localhost/assignment1/fall20Testing.txt";
        ...
        String header = "<html>" + EOL + "  <head>" + EOL + "    <title>CS 4339/5339 PHP assignment</title>" + EOL
                + "  </head>" + EOL + "  <body>" + EOL + "    <pre>";
        String footer = "    </pre>" + EOL + "  </body>" + EOL + "</html>";
        String inputLine;
        String inputFile = "";
